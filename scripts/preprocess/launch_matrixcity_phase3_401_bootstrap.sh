...
OUT_DIR="${OUT_DIR:-/vePFS-buaa/linming/workspace/worldrender/MATRIXCITY_PARQUET_PHASE3_401_BOOTSTRAP_W0CTRL_DN401}"
...
POSE_FILTER_TRANSLATION_RATIO="${POSE_FILTER_TRANSLATION_RATIO:-0}"
POSE_FILTER_MAX_ROTATION_DEG="${POSE_FILTER_MAX_ROTATION_DEG:-0}"
...
# Notes:
# 1. This launch writes a lightweight phase-3 bootstrap parquet.
# 2. The parquet keeps text / first_frame_latent / metadata and additionally
#    caches the first 0-80 window control_latent (depth + normal + maskrgb + mask).
# 3. depth/normal branch latents are cached for the full 401-frame clip so
#    later rollout windows can reuse them directly during training.
# 4. RAW_ROOT is still required because training-time online warp keeps
#    generating mask/maskrgb for later windows from raw RGB/depth/camera.
