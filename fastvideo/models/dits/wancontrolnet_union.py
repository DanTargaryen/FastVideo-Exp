...
class WanControlnetUnion3DModel(BaseDiT):
    """
    Bidirectional Union ControlNet for video latents.

    Expected inputs:
    - `hidden_states`: (B, C_lat, F, H, W)
    - `mask`: (B, C_lat, F, H, W)
    - `masked_latent`: (B, C_lat, F, H, W)
    - `controlnet_cond`: WanControlNetUnionInput (depth/normal)
    """
    ...
    def __init__(self, config: WanVideoConfig,
        ...
        # 1. Patch embedding (expects concatenated [latent, mask, masked_latent])
    ...
    def forward(
        ...
        **_kwargs,
        ...
        ts_seq_len = int(timestep_seq.shape[1]) if timestep_seq.dim(
        ) == 2 else None
        temb, timestep_proj, encoder_hidden_states, encoder_hidden_states_image = self.condition_embedder(
            timestep_seq.flatten(),
            encoder_hidden_states,
        ...
        if timestep_proj.dim() == 3:
            timestep_proj = timestep_proj + control_global_emb[:, None, :]
        else:
